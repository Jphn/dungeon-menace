...
public class UI {
    ...
    public void drawUsernameScreen() {
        ...
        for (Character letra : gp.username) {
            concName += letra;
        }
    ...
    public void drawScoreboardScreen() {
        ...
        gp.requestCounter++;
        if (gp.requestCounter < 808 && !this.scores.isEmpty()) {
            y = gp.tileSize * 4;
            ...
            for (Score score : this.scores) {
                ...
                String message = score.getUsername() + " | " + score.getScore();

                g2.drawString(message, getXforCenteredText(message), y);
            ...
            try {
                this.scores = this.gp.scoreboard.get();

                this.scores.sort((Score a, Score b) -> {
                    return (int) a.getScore() - (int) b.getScore();
                });

                this.scores = this.scores.reversed();
            } catch (IOException e) {
